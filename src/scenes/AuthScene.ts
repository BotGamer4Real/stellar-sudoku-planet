...
import { getCurrentUser, signUpWithEmailAndUsername, signInWithEmail, resetPassword } from '../services/supabaseClient';
...
export class AuthScene extends Phaser.Scene {
  ...
  create(): void {
    ...
    this.usernameInput = this.formContainer.querySelector('#username') as HTMLInputElement;
    ...
    console.log('%c🔐 AuthScene loaded with username support', 'color: cyan; font-size: 14px');
  }
  ...
  private async handleMainAction(): Promise<void> {
    ...
    if (this.isSignUpMode) {
      ...
      const result = await signUpWithEmailAndUsername(email, password, username);
      if (result.error) {
        console.error('%c❌ Sign-up failed:', 'color: red', result.error.message);
        window.alert('Sign-up failed: ' + result.error.message);
      } else {
        console.log('%c✅ Account created with username — check your email', 'color: lime');
